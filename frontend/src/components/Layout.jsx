...
function Layout({children, activeTab}) {
    ...
    const handleTextSubmit = async (e) => {
        ...
        const response = await fetch('/chit/', {
            method: 'POST',
        ...
        <div className="flex h-screen overflow-hidden dark:text-white dark:bg-stone-900">
            ...
            <div className="side w-1/3 sticky top-0 flex flex-col">
                <div className='flex h-16 shrink-0 items-center py-2 px-2'>
                    ...
                    <div className='m-2'><a target='blank' href='https://github.com/pradyun1611'><svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6"><path stroke-linecap="round" stroke-linejoin="round" d="m6.75 7.5 3 2.25-3 2.25m4.5 0h3m-9 8.25h13.5A2.25 2.25 0 0 0 21 18V6a2.25 2.25 0 0 0-2.25-2.25H5.25A2.25 2.25 0 0 0 3 6v12a2.25 2.25 0 0 0 2.25 2.25Z" /></svg></a></div>
                    {user ? (
                        <>
                        <Link className='h-full m-2 flex items-center justify-center' to={`/user/${curUser._id}`}><img src={`http://localhost:4000/pfp/${curUser.pfp}`} alt='pfp' className='h-7 w-7 object-cover rounded-full'/></Link>
                    ...
                    {searching ? (
                        <div>
                            {filteredUsers.map(user => (
                                ...
                                pfp={`http://localhost:4000/pfp/${user.pfp}`}
                                />
                            ))}
                        </div>
                        ) : (
                        <>
                            ...
                            <div className='flex flex-col dark:bg-zinc-800 border border-yellow-500 m-2 pb-2 rounded-xl'>
                                {users && users.filter((u) => u.username !== user.username).sort((a, b) => b.followers.length - a.followers.length).slice(0, 5).map((user) => (
                                    <Account id={user._id} name={user.name} username={user.username} pfp={`http://localhost:4000/pfp/${user.pfp}`} />
                                ))}
                            </div>
                        </>
                    )}
