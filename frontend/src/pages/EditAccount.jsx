...
function EditAccount() {
    ...
    const updateUser = async (name, username, dob, location) => {
        const response = await fetch(`/user/${curUser._id}`, {
            method: 'PATCH',
            headers: {
    ...
    const handleImageUpload = async () => {
        ...
        const response = await fetch(`/user/upload/${curUser._id}`, {
            method: 'PATCH',
            headers: {
                'Authorization': `Bearer ${user.token}`,
            },
        ...
        <Layout>
            <div className="h-screen w-full flex justify-center">
                <form onSubmit={handleSubmit} className="flex flex-col h-4/5 w-1/2 items-center justify-center">
                    <img className="object-cover mb-5 h-32 w-32 rounded-full" src={`http://localhost:4000/pfp/${curUser.pfp}`} alt="pfp" />
